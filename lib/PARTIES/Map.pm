...
sub finish {
  ...
  $self->stdlog("$bowtie2 --threads $self->{THREADS} --local -x $self->{BT2_INDEX} -1 $self->{FASTQ1} -2 $self->{FASTQ2} -X $self->{MAX_INSERT_SIZE} | $samtools view -uS - | $samtools sort - -o $out_bam > /dev/null 2>&1\n");
